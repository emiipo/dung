...
std::vector<float> LevelGenerator::GetGradient(int x, int y){
...
float LevelGenerator::GenerateNoise(float x, float y){
    int x0 = floor(x);
    int y0 = floor(y);
    int x1 = x0+1;
    int y1 = y0+1;
...
float LevelGenerator::DotGridGradient(int gx, int gy, float x, float y){
...
float LevelGenerator::Interpolate(float a, float b, float w){
